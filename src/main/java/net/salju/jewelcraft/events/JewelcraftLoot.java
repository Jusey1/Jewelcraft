...
public class JewelcraftLoot {
	...
	public void onLoot(LootTableLoadEvent event) {
		if (event.getName().equals(BuiltInLootTables.SHIPWRECK_TREASURE)) {
			...
			LootPool unique = JewelcraftHelpers.setUnique("captain_loot", -3.0F, 1.0F,
			...
			LootTable table = event.getTable();
			LootPool unique = JewelcraftHelpers.setUnique("jungle_loot", -3.0F, 1.0F,
			...
			LootTable table = event.getTable();
			LootPool unique = JewelcraftHelpers.setUnique("desert_loot", -3.0F, 1.0F,
			...
			LootTable table = event.getTable();
			LootPool unique = JewelcraftHelpers.setUnique("stronghold_loot", -3.0F, 1.0F,
					JewelcraftHelpers.getUnique(JewelryItems.IRON_RING_LAPIS.get(), "item.jewelcraft.kyan_ring", JewelryEnchantments.KYANITE.get(), null));
			table.addPool(unique);
			event.setTable(table);
		} else if (event.getName().equals(BuiltInLootTables.FISHING_TREASURE)) {
			LootTable table = event.getTable();
			LootPool unique = JewelcraftHelpers.setUnique("fish_loot", -24.0F, 1.0F,
			...
			LootTable table = event.getTable();
			LootPool unique = JewelcraftHelpers.setUnique("spawnbonus_loot", 1.0F, 1.0F,
					JewelcraftHelpers.getUnique(JewelryItems.IRON_AMULET_EMERALD.get(), "item.jewelcraft.bonus_amulet", JewelryEnchantments.SATISFICATION.get(), null));
			table.addPool(unique);
			event.setTable(table);
		} else if (JewelcraftHelpers.getVillages().contains(event.getName())) {
			LootTable table = event.getTable();
			LootPool unique = JewelcraftHelpers.setUnique("villager_loot", -4.0F, 1.0F,
